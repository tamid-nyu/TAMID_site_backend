...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    try {
      ...
      // Configured but unreachable — usually an expired token. Report it as a
      // state the panel can render rather than as a request failure.
      const message = error instanceof InstagramError ? error.message : 'Unknown error';
      res.json({ success: true, data: { connected: false, error: message } });
    }
  })
);
